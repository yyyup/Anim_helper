...
# move each F-curve for the object's location by 2 frames
for fcurve in fcurves:
    if fcurve.data_path == "location":
...
# Remove rotation and scale F-curves for each empty
for empty in created_empties:
    ...
    if empty_animation_data is not None:
        ...
        if empty_action is not None:
            for fcurve in empty_action.fcurves:
                if fcurve.data_path.startswith("rotation") or fcurve.data_path.startswith("scale"):
                    empty_action.fcurves.remove(fcurve)